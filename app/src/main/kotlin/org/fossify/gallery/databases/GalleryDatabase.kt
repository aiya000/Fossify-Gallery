...
@Database(entities = [Directory::class, Medium::class, Widget::class, DateTaken::class, Favorite::class, PCloudItem::class], version = 11)
abstract class GalleryDatabase : RoomDatabase() {
    ...
    abstract fun PCloudItemDao(): PCloudItemDao
    ...
    companion object {
        ...
        private val MIGRATION_10_11 = object : Migration(10, 11) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("CREATE TABLE IF NOT EXISTS `pcloud_items` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `path` TEXT NOT NULL, `item_id` INTEGER NOT NULL, `is_folder` INTEGER NOT NULL, `content_hash` INTEGER NOT NULL, `has_thumb` INTEGER NOT NULL, `last_scanned_at` INTEGER NOT NULL)")
                database.execSQL("CREATE UNIQUE INDEX `index_pcloud_items_path` ON `pcloud_items` (`path`)")
            }
        }
    }
}
