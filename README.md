...
> installed by hand. The store badges below lead to the official app, which does not contain the
...
- The foot of the settings screen says which build this is, in small print: the release tag alone when the build is of a tagged commit, otherwise the nearest tag with the commits since it and the sha (`-dirty` for uncommitted changes), the branch, and when it was built. Gradle reads it off git at build time, see `BUILD_LABEL` in `app/build.gradle.kts`
...
- Deleting, renaming and creating folders act on pCloud itself. A deleted file goes to the gallery's own recycle bin on pCloud (a hidden `.gallery-recycle-bin` folder in the root), and is listed in the folder list's one recycle bin together with what was deleted on this device and on the network share; it can be restored from there to the folder it came from, or to another pCloud folder, for 30 days. Deleting with "skip the recycle bin", or from the bin, sends the file to the trash on pcloud.com, which the app cannot read: pCloud does not open its trash to OAuth apps
